...
class Toolbox(object):
    def __init__(self):
        ...
        self.tools = [Reconstruction_of_watercourses, Reconstruction_of_avalanche_path_components] 
...
class Reconstruction_of_watercourses(object):
    ...
    def execute(self, parameters, messages):
        ...
        out_stream_links = arcpy.Dissolve_management('stream_dissolve', out_stream_links, "", "", "MULTI_PART", "UNSPLIT_LINES")


class Reconstruction_of_avalanche_path_components(object):
    def __init__(self):
        self.label       = "Reconstruction of avalanche path components"
        self.description = "Reconstruction of avalanche path components"
...
# Этап 1. ПОИСК ТОЧКИ В 20 ГРАДУСОВ
        ...
        while True:
            ...
            if values_len == count_vertex:
                ...
                if points_upper[-1] == count_vertex:
                ...
                if points_below[0] == 0:
                    with arcpy.da.SearchCursor('dens_copy_1', 'SHAPE@') as cursor:
                        for row in cursor:
                    ...
                    with arcpy.da.SearchCursor('split_lines_3', ['SHAPE@', 'OID@']) as cursor:
                        for row in cursor:
                            ...
                            if (coords_start != split_start):
                                objval_1 = row[1]
                                break
                    query_line_2 = '"OBJECTID" = {0}'.format(objval_1)
                elif points_upper[-1] == count_vertex:
                    with arcpy.da.SearchCursor('dens_copy_1', 'SHAPE@') as cursor:
                        for row in cursor:
                            coords_start = tuple((row[0].firstPoint.X, row[0].firstPoint.Y))
                            break
                ...
                arcpy.MakeFeatureLayer_management('split_lines_3', "select_line", query_line_2) 
  ...
  # 2.5.5.обработка выбранной линии
                ...
                if value_len_line < value_thresh:
                    point_query = '"OBJECTID" = 1'
                    arcpy.MakeFeatureLayer_management('point_select_line', "point_zone_20", point_query)
                    arcpy.AddMessage('step 2 of 15')
                else:
                    ...
                    arcpy.Merge_management(point_list_20, "point_thr_ext_20") #упал астер 22
...
# ПОИСК ПОЛИГОНА 3ОНЫ 3АРОЖДЕНИЯ
        ...
        if cell_p > 10:
            remap = arcpy.sa.RemapRange([[0,2500,1],[2500,6000,0],[6000,9000,0]])
        else:
        ...
        arcpy.analysis.Intersect(["slope_line_dis", 'dens_copy_2'] , "inter_slope_25_to_split", "", "", "POINT")
        arcpy.management.SplitLineAtPoint('dens_copy_2', 'inter_slope_25_to_split', "stream_25_line_split", "1 Meters")
        ...
        out_watershed_raster = arcpy.sa.Watershed(flow_directions, 'aval_tranz_zone')
        arcpy.conversion.RasterToPolygon(out_watershed_raster, "out_watershed_polygon", "NO_SIMPLIFY", "Value")
        if cell_p > 10:
            arcpy.analysis.Clip('out_watershed_polygon', 'slope_poly_25', "clip_polygon_wsh")
            arcpy.analysis.SymDiff('out_watershed_polygon', 'clip_polygon_wsh', "watershed_output_fc")
            arcpy.management.AddGeometryAttributes('watershed_output_fc', "AREA", "", "SQUARE_METERS")
            watershed_output = arcpy.MakeFeatureLayer_management('watershed_output_fc', watershed_output)
        ...
        arcpy.AddMessage('step 4 of 15')


# ПОИСК ТОЧКИ 25 ГРАДУСОВ
        # Разворачиваем линию так, чтобы счет шел снизу вверх, а не сверху вниз
        arcpy.FeatureVerticesToPoints_management('dissolve_stream', "aval_tranz_zone_s_e", "BOTH_ENDS")
        arcpy.sa.ExtractValuesToPoints('aval_tranz_zone_s_e', DEM_fill, "aval_tranz_zone_s_e_value")
        s_e_aval = [i[0] for i in arcpy.da.SearchCursor('aval_tranz_zone_s_e_value', "RASTERVALU")]
        razn_s_e_aval = s_e_aval[0] - s_e_aval[1]
        if razn_s_e_aval > 0:
            aval_tranz_zone_flip = arcpy.edit.FlipLine('dissolve_stream')
        else:
            aval_tranz_zone_flip = arcpy.MakeFeatureLayer_management('dissolve_stream', aval_tranz_zone_flip)
        ...
        px_i_25 = [i[0] for i in arcpy.da.SearchCursor('intersect_point_25_sing', "POINT_X")][-1]#по 0,5 упало тут, по 5м тоже
        ...
        if len(inter_point_1) == 1:
            ...
            for i in inter_point:
                ...
                if i < len(inter_point) - 1:
                    query_inter_poin = '"OBJECTID" = {0} OR "OBJECTID" = {1}'.format((i),(i+1))
                    ...
                    len_inter_stream = [z[0] for z in arcpy.da.SearchCursor('i_stream', "OBJECTID")]
                    if len_inter_stream == 3:
                ...
                # если точка последняя, то режем по ней и берем первую линию
                    ...
                    query_inter_poin_1 = '"OBJECTID" = {0}'.format(i)
                    arcpy.MakeFeatureLayer_management('intersect_point_25_sing', "i_point", query_inter_poin_1)
                    arcpy.management.SplitLineAtPoint(aval_tranz_zone_flip, 'i_point', "i_stream", "1 Meters")
                    with arcpy.da.SearchCursor(aval_tranz_zone_flip, 'SHAPE@') as cursor:
                        for row in cursor:
                ...
                len_i_line = [k[0] for k in arcpy.da.SearchCursor('i_stream_interest', "LENGTH")][0]#тут поломался алос 43
                # если длина выбранной линии больше порога, то заходим в цикл
                if len_i_line >= len_treshold_line:
                    point_i = 'point_i_%s' % (i)
                    query_inter_poin_interest = '"OBJECTID" = {0}'.format(i)
                ...
                if point_list_len != 0:
                    ...
                    query_point_25 =  '"OBJECTID" = {0}'.format(inter_point_1[0])
                    arcpy.MakeFeatureLayer_management('intersect_point_25_sing', "point_25_degree_end", query_point_25)
    ...
    # находим верхнюю точку НЕ перевернутой линии
        arcpy.FeatureVerticesToPoints_management('dens_copy_2', "start_end_point", "BOTH_ENDS")
        ...
        if len_inter_0 == 1:
            ...
            arcpy.management.AddGeometryAttributes('dissolve_stream', "LENGTH","METERS")               
            ...
            arcpy.MakeFeatureLayer_management('line_25_degree', "line_before_sz", query_line_25_before)
            arcpy.management.AddGeometryAttributes('line_before_sz', "LENGTH","METERS")
            len_slope_ds = [i[0] for i in arcpy.da.SearchCursor('dissolve_stream', "LENGTH")][0]
            len_slope_berore_sz = [i[0] for i in arcpy.da.SearchCursor('line_before_sz', "LENGTH")][0]
            len_thresh_dis = len_slope_ds * 15 / 100
            if len_slope_berore_sz >= len_thresh_dis:
                ...
                with arcpy.da.SearchCursor('line_25_degree', ['SHAPE@', 'OID@']) as cur_1:
                    for row in cur_1:
                        ...
                        if (coords_start != split_start):
                            objval_2 = row[1]
                            break
                query_line_25_after = '"OBJECTID" = {0}'.format(objval_2)
                arcpy.MakeFeatureLayer_management('line_25_degree', "line_to_wsh", query_line_25_after)
        else:
        ...
        arcpy.AddMessage('step 5 of 15')



# ПОИСК ПОЛИГОНА 3ОНЫ ТРАН3ИТА
        if cell_p > 10:
            remap_20 = arcpy.sa.RemapRange([[0,2000,1],[2000,6000,0],[6000,9000,0]])
        else:
        ...
        arcpy.Dissolve_management('slope_line_20', "slope_line_dis_20", "", "", "MULTI_PART", "UNSPLIT_LINES")
        arcpy.analysis.Intersect(["slope_line_dis_20", 'dens_copy_2'] , "inter_slope_20_to_split", "", "", "POINT")
        arcpy.management.SplitLineAtPoint('dens_copy_2', 'inter_slope_20_to_split', "stream_20_line_split", "1 Meters")
        ...
        # 3.2.1. создание растра водораздела и конвертация его в вектор
        out_watershed_raster_20 = arcpy.sa.Watershed(flow_directions, 'aval_tranz_zone')
        arcpy.conversion.RasterToPolygon(out_watershed_raster_20, "out_watershed_polygon_20", "NO_SIMPLIFY", "Value")
        if cell_p > 10:
            arcpy.analysis.Clip('out_watershed_polygon_20', 'slope_poly_20', "clip_polygon_trz")
            arcpy.analysis.SymDiff('out_watershed_polygon_20', 'clip_polygon_trz', "watershed_tranz_fc")
            arcpy.analysis.SymDiff('out_watershed_polygon_20', watershed_output, "watershed_tranz_fc_1")
            arcpy.MakeFeatureLayer_management('watershed_tranz_fc_1', "zone_polygon_all")
        else:
        ...
        arcpy.analysis.Intersect(['zone_polygon_all', 'dens_copy_2'], "point_int_zone_polygon_multi", "", "", "POINT")
        ...
        point_2_down_count = len([i[0] for i in arcpy.da.SearchCursor('point_int_zone_polygon_all_height_sort', "OBJECTID")])
        if point_2_down_count == 0:
            arcpy.analysis.Intersect([watershed_output, 'slope_line_dis'] , "down_line_start_zone", "", "", "LINE")
            arcpy.management.MultipartToSinglepart('down_line_start_zone', "down_line_start_zone_single")
            arcpy.Dissolve_management('down_line_start_zone_single', "down_line_start_zone_diss","", "", "SINGLE_PART", "DISSOLVE_LINES")
            arcpy.FeatureVerticesToPoints_management('down_line_start_zone_diss', "down_line_start_zone_diss_point", "BOTH_ENDS")
            query_point_dow_zone_1 = '"OBJECTID" =  1'
            query_point_dow_zone_2 = '"OBJECTID" =  2'
            arcpy.MakeFeatureLayer_management('down_line_start_zone_diss_point', "point_down_line_1", query_point_dow_zone_1)
            arcpy.MakeFeatureLayer_management('down_line_start_zone_diss_point', "point_down_line_2", query_point_dow_zone_2)
            merge_point_list_1 = ['20_degree', 'point_down_line_1']
            merge_point_list_2 = ['20_degree', 'point_down_line_2']
            arcpy.Merge_management(merge_point_list_1, "point_20_down_1")
            arcpy.Merge_management(merge_point_list_2, "point_20_down_2")
            arcpy.management.PointsToLine('point_20_down_1', "line_20_down_1")
            arcpy.management.PointsToLine('point_20_down_2', "line_20_down_2")
            merge_tr_line = ['down_line_start_zone_diss', 'line_20_down_1', 'line_20_down_2']
            arcpy.Merge_management(merge_tr_line, "line_tranzit_zone")
            arcpy.management.FeatureToPolygon('line_tranzit_zone', "tranzit_zone_polygon_fc")
            tranzit_zone_polygon =  arcpy.MakeFeatureLayer_management('tranzit_zone_polygon_fc', tranzit_zone_polygon)
...
# ВЫДЕЛЕНИЕ ОТДЕЛЬНЫХ ЛИНИЙ
        ...
        arcpy.FeatureVerticesToPoints_management('line_to_wsh', "start_end_point_avalanche", "BOTH_ENDS")
        arcpy.sa.ExtractValuesToPoints('start_end_point_avalanche', DEM_fill, "start_end_point_avalanche_height")
        s_e_points_aval = [i[0] for i in arcpy.da.SearchCursor('start_end_point_avalanche_height', "RASTERVALU")]
        arcpy.management.AddGeometryAttributes('start_end_point_avalanche_height', "POINT_X_Y_Z_M","METERS")

        razn_z_start_end_aval = s_e_points_aval[0]-s_e_points_aval[1]
        if razn_z_start_end_aval < 0:
            arcpy.edit.FlipLine('line_to_wsh')
            x_start = [k[0] for k in arcpy.da.SearchCursor('start_end_point_avalanche_height', "POINT_X")][-1]
            y_start = [k[0] for k in arcpy.da.SearchCursor('start_end_point_avalanche_height', "POINT_Y")][-1]
        else:
            x_start = [k[0] for k in arcpy.da.SearchCursor('start_end_point_avalanche_height', "POINT_X")][0]
            y_start = [k[0] for k in arcpy.da.SearchCursor('start_end_point_avalanche_height', "POINT_Y")][0]
...
# ПРОДЛЯЕМ ЛИНИЮ 3ОНЫ ВЫХОДА, ЕСЛИ ОНА БОЛЬШЕ ТРИДЦАТИ ГРАДУСОВ
        if len_path_comp_count == 4:
            ...
            arcpy.management.AddGeometryAttributes('30_degree', "POINT_X_Y_Z_M","METERS")
            ...
            px_r = [k[0] for k in arcpy.da.SearchCursor('30_degree', "POINT_X")][0]
            py_r = [k[0] for k in arcpy.da.SearchCursor('30_degree', "POINT_Y")][0]
...
# ПОИСК ТАНГЕНСА УГЛА НАКЛОНА
        ...
        arcpy.AddMessage('step 11 of 15')
...
# ПОИСК ТОЧКИ ОКОНЧАНИЯ
        ...
        arcpy.FeatureVerticesToPoints_management(start_track, "start_track_poin_aval", "ALL")
        ...
        point_end_aval_len = len([angle_psi.index(i) for i in angle_psi if i <= tan_psi_threshold])
        if point_end_aval_len != 0:
            ...
            point_end_aval_query = '"OBJECTID" = {0}'.format(point_qery_list[-1])
            arcpy.MakeFeatureLayer_management('runout_track_point_value', "point_avalance_end", point_end_aval_query) #тут тоже поломалось
        arcpy.AddMessage('step 12 of 15')
...
# ПОИСК ЛИНИИ ЗОНЫ ОСАЖДЕНИЯ
        ...
        arcpy.AddMessage('step 13 of 15')
...
# ПОИСК ПОЛИГОНА ЗОНЫ ОСАЖДЕНИЯ
        ...
        arcpy.management.FeatureToPolygon('runout_line', "runout_zone_polygon") #поломалось
        ...
        arcpy.AddMessage('step 14 of 15')



        arcpy.FeatureVerticesToPoints_management(tranzit_track, "new_20_25_degre_new", "ALL")
        
        i_25_point = [k[0] for k in arcpy.da.SearchCursor('new_20_25_degre_new', "OBJECTID")][0]
        i_20_point = [k[0] for k in arcpy.da.SearchCursor('new_20_25_degre_new', "OBJECTID")][-1]

        query_p20 =  '"OBJECTID" = {0}'.format(i_20_point)
        query_p25 =  '"OBJECTID" = {0}'.format(i_25_point)
        arcpy.MakeFeatureLayer_management('new_20_25_degre_new',"point_25_new",query_p25)
        arcpy.MakeFeatureLayer_management('new_20_25_degre_new',"point_20_new",query_p20)




#ПОИСК ЗНАЧЕНИЯ СКОРОСТИ ЛАВИНЫ ДЛЯ ТОЧКИ В КОНЦЕ ЗОНЫ ЗАРОЖДЕНИЯ
        arcpy.sa.ExtractValuesToPoints('point_25_new', DEM_fill, "point_25_new_value")
        z_25_point = [k[0] for k in arcpy.da.SearchCursor('point_25_new_value', "RASTERVALU")][0]
...
#ПОИСК ЗНАЧЕНИЯ СКОРОСТИ ЛАВИНЫ ДЛЯ ТОЧКИ В КОНЦЕ ЗОНЫ ТРАНЗИТА
        arcpy.sa.ExtractValuesToPoints('point_20_new', DEM_fill, "point_20_new_value")
        z_20_point = [k[0] for k in arcpy.da.SearchCursor('point_20_new_value', "RASTERVALU")][0]
        ...
        arcpy.AddMessage('step 15 of 15')
...
# ПОДГОТОВКА ДАННЫХ ДЛЯ ЗАПИСИ В ТЕКСТОРВЫЙ ФАЙЛ
        ...
        arcpy.management.AddGeometryAttributes('point_25_new', "POINT_X_Y_Z_M","METERS")
        coords_25_x = [k[0] for k in arcpy.da.SearchCursor('point_25_new', "POINT_X")][0]
        coords_25_y = [k[0] for k in arcpy.da.SearchCursor('point_25_new', "POINT_Y")][0]

        arcpy.management.AddGeometryAttributes('point_20_new', "POINT_X_Y_Z_M","METERS")
        coords_20_x = [k[0] for k in arcpy.da.SearchCursor('point_20_new', "POINT_X")][0]
        coords_20_y = [k[0] for k in arcpy.da.SearchCursor('point_20_new', "POINT_Y")][0]
