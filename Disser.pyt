...
class Reconstruction_of_watershades(object):
    ...
    def execute(self, parameters, messages):
        ...
        boolval = True
        slope_threshold_avalance_zone = 21
        slope_threshold_tranzit_zone = 25
        k = 1
        ...
        while True:
            ...
            points_upper = [values_input.index(i) for i in values_input if i > 19]
            points_below = [values_input.index(i) for i in values_input if i <= 25]
            ...
            if values_len == count_vertex:
                ...
                points_max_25 = [values_slope.index(i) for i in values_slope if i > 25]
                ...
                for i in range(len(points_max_25)):
                    query_len_slope = '"OBJECTID" = {0}'.format(points_max_25[i])
                    point_extr_25 = 'point_%s_extr_25' % (i+1)
                    arcpy.MakeFeatureLayer_management('select_point_value_slope', point_extr_25, query_len_slope)
                    point_list_25.append(point_extr_25)
                # 2.5.5.5. объединение точек перегиба
                arcpy.Merge_management(point_list_20, "point_thr_ext_20")
                arcpy.Merge_management(point_list_25, "point_thr_ext_25")
                ...
                arcpy.management.SplitLineAtPoint('select_line', 'point_thr_ext_25', "line_thr_ext_25", "1 Meters")
                arcpy.management.AddGeometryAttributes('line_thr_ext_25', "LENGTH","METERS")
                ...
                arcpy.management.Sort('line_thr_ext_25', "line_thr_ext_sort_25", [["LENGTH", "DESCENDING"]])
                values_slope_max_25 = [i[0] for i in arcpy.da.SearchCursor('line_thr_ext_sort_25', "LENGTH")]
                value_slope_max_25 = values_slope_max_25[0]
                percent_line_25 = value_slope_max_25*100/value_len_line
                # 2.5.5.7. есть два преобладающих отрезка
                if percent_line_20 >= 20 and percent_line_25 >= 50:
                    arcpy.AddMessage('use percent_line_20 >= 20 and percent_line_25 >= 50')
                    query_line_20_25 = '"OBJECTID" = 1'
                    arcpy.MakeFeatureLayer_management('line_thr_ext_sort_20', "line_thr_min_20", query_line_20_25)
                    arcpy.MakeFeatureLayer_management('line_thr_ext_sort_25', "line_thr_min_25", query_line_20_25)
                    ...
                    arcpy.FeatureVerticesToPoints_management('line_thr_min_25', "point_thr_start_end_25", "BOTH_ENDS")
                    arcpy.sa.ExtractValuesToPoints('point_thr_start_end_25', DEM_fill, "point_thr_start_end_height_25")
                    arcpy.management.Sort('point_thr_start_end_height_25', "point_thr_start_end_height_sort_25", [["RASTERVALU", "DESCENDING"]])
                    ...
                    arcpy.MakeFeatureLayer_management('point_thr_start_end_height_sort_25', "point_zone_25", query_point_20)
                ...
                if percent_line_20 >= 20 and percent_line_25 < 50:
                    ...
                    point_max = [angle.index(i) for i in angle if i <= 25][1]
                ...
                if percent_line_20 < 20 and percent_line_25 < 50:
                    ...
                    arcpy.AddMessage(angle)
                    point_max_20 = [angle.index(i) for i in angle if i <= 21][1]
                    query_len_slope_20 = '"OBJECTID" = {0}'.format(point_max_20)
                    point_max_25 = [angle.index(i) for i in angle if i <= 25][1]
                    query_len_slope_25 = '"OBJECTID" = {0}'.format(point_max_25)
                    arcpy.MakeFeatureLayer_management('select_point_value_slope', "point_zone_20", query_len_slope_20)
                    arcpy.AddMessage('Slope 20 ok')
                    arcpy.MakeFeatureLayer_management('select_point_value_slope', "point_zone_25", query_len_slope_25)
                    arcpy.AddMessage('Slope 25 ok')


                    # arcpy.Delete_management('point_thr_ext')
                    # arcpy.Delete_management('line_thr_ext')
                    # arcpy.Delete_management('line_thr_ext_sort')
                    # arcpy.Delete_management('line_thr_min')
                    # arcpy.Delete_management('point_thr_start_end')
                    # arcpy.Delete_management('point_thr_start_end_height')
                    # arcpy.Delete_management('point_thr_start_end_height_sort')
                    # arcpy.Delete_management('select_point_value')
                    # arcpy.Delete_management('current_points')
                    # arcpy.Delete_management('splitted_fclass')   

                count_point_zone_20 = arcpy.management.GetCount('point_zone_20')
                arcpy.AddMessage(count_point_zone_20) 

                count_point_zone_25 = arcpy.management.GetCount('point_zone_25')
                arcpy.AddMessage(count_point_zone_25)   

                point_output = arcpy.MakeFeatureLayer_management('point_zone_20', point_output)
                aval_tranz_zone = arcpy.MakeFeatureLayer_management('point_zone_25', aval_tranz_zone)

                point_20_25 = ['point_zone_20', 'point_zone_25']
                arcpy.Merge_management(point_20_25, "point_intersect_20_25_degree") #"point_intersect_20_25_degree"

                count_point_zone_25_20 = arcpy.management.GetCount('point_intersect_20_25_degree')
                arcpy.AddMessage(count_point_zone_25_20)            

                #aval_tranz_zone = arcpy.management.SplitLineAtPoint('dissolve_stream', point_output, aval_tranz_zone, "1 Meters")  
                
                # 2.5.5.8.9. разрезание линии по выбранной точке
                # arcpy.management.SplitLineAtPoint('dissolve_stream', 'point_zone_20', "line_with_end_point", "1 Meters")
